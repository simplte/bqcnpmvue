class Pinyin {
  ...
  zhuanpingyin() {
    let self = this;
    let enameList = self.createEnglishName().namePinyin(this.name);
    // // 清空数据
    // self.passgerInfo.exing = "";
    // self.passgerInfo.ename = "";
    // self.pyShowList.xingS = [];
    // self.pyShowList.mingS = [];
    // self.pyXList = [];
    // self.pyMList = [];
    // self.controlProp.xingC = [];
    // self.controlProp.mingC = [];
    // //处理名字
    // if (enameList && enameList.I2 && enameList.I2.length) {
    //   let list1 = [];
    //   enameList.I2.forEach(item => {
    //     if (item.length == 0) return;
    //     if (item.length == 1) self.pyShowList.mingS.push(item[0].val);
    //     if (item.length > 1) {
    //       self.controlProp.mingC.push({
    //         isShow: true
    //       });
    //       self.pyShowList.mingS.push('/');
    //       list1.push(item);
    //     }
    //   });
    //   self.pyMList = list1.reverse();
    // }
    // // 处理姓
    // if (enameList && enameList.I1 && enameList.I1.length) {
    //   let list = [];
    //   enameList.I1.forEach(item => {
    //     if (item.length == 0) return;
    //     if (item.length == 1) self.pyShowList.xingS.push(item[0].val);
    //     if (item.length > 1) {
    //       self.controlProp.xingC.push({
    //         isShow: true
    //       });
    //       self.pyShowList.xingS.push('/');
    //       list.push(item);
    //     }
    //   });
    //   self.pyXList = list.reverse();
    // }
    // debugger
    // // 判断是否存在多音字,没有多音字直接复制拼音
    // if (self.pyMList.length || self.pyXList.length) {
    //   self.propBgShow = true;
    // } else {
    //   if (self.pyShowList.xingS.length > 0) {
    //     self.pyShowList.xingS.forEach(item => {
    //       self.passgerInfo.exing += item;
    //     });
    //   }
    //   if (self.pyShowList.mingS.length > 0) {
    //     self.pyShowList.mingS.forEach(item => {
    //       self.passgerInfo.ename += item;
    //     });
    //   }
    // }
    return enameList;
  ...
  createEnglishName() {
    return {
      ...
      ucfirst: function (l1) {
        ...
        if (l1.length > 0) {
          l1.forEach(item => {
            var first = item.substr(0, 1).toUpperCase();
            var spare = item.substr(1, l1.length);
            pinyinUpperCase.push(first + spare);
          });
        }
        return pinyinUpperCase;
      },
      namePinyin: function (string) {
        let that = this;
        let str1 = string.substr(0, 2);
        let str2 = "";
        if (that.hyphenated[str1]) {
          str2 = string.substr(2);
        } else {
          str1 = string.substr(0, 1);
          str2 = string.substr(1);
        }
        ...
        for (let i = 0; i < len1; i++) {
          let val = str1.substr(i, 1);
          let name = that.arraySearch(val, that.PinYin);
          if (reg.test(val)) {
            I1.push([{
              'font': val,
              'val': val.toUpperCase()
